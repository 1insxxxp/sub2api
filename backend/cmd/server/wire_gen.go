...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userCustomGroupRepository := repository.NewUserCustomGroupRepository(client)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userCustomGroupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService)
	...
	userCustomGroupService := service.NewUserCustomGroupService(userCustomGroupRepository, userRepository, groupRepository, gatewayService)
	...
	systemCustomGroupService := service.NewSystemCustomGroupService(systemCustomGroupRepository, groupRepository, gatewayService)
	systemCustomGroupHandler := admin.NewSystemCustomGroupHandler(systemCustomGroupService)
	adminHandlers := handler.ProvideAdminHandlersWithSystemCustomGroup(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, checkinHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, systemCustomGroupHandler)
