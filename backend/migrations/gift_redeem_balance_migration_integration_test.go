...
WHERE conname = 'users_gift_balance_within_balance'
  AND conrelid = 'users'::regclass`).Scan(&relationValidated))
	require.False(t, relationValidated, "234 must commit the short ACCESS EXCLUSIVE lock before validation")

	content235, err := FS.ReadFile("235_repair_and_validate_gift_balance.sql")
	require.NoError(t, err)
	for range 2 {
		_, err = db.ExecContext(ctx, string(content235))
		require.NoError(t, err)
	}
