...
// EffectiveModelAllowlist returns the canonical allowlist for this group.
// ModelAllowlist is preferred, but legacy ModelsListConfig is still honored.
func (g *Group) EffectiveModelAllowlist() GroupModelAllowlist {
	if g == nil {
	...
	return GroupModelAllowlist(g.ModelsListConfig)
}

func syncGroupModelAllowlistAliases(group *Group) {
	if group == nil {
		return
	}
	effective := group.EffectiveModelAllowlist()
	group.ModelAllowlist = effective
	group.ModelsListConfig = GroupModelsListConfig(effective)
}
...
func (g *Group) ModelAllowlistEnabled() bool {
	return g != nil && g.EffectiveModelAllowlist().Enabled
}
