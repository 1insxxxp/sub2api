...
// LegacyLongContextRule is the legacy platform-specific long-context policy.
// Only Gemini's native v1beta path uses it when no explicit group/channel price applies.
...
// LegacyLongContextRule returns a platform's legacy long-context rule.
...
type TokenCostRequest struct {
	...
	// LegacyLongContext is used only by the Gemini compatibility entrypoint.
