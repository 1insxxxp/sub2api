...
func (s *OpenAIGatewayService) IsSystemCustomAccountRuntimeEligible(ctx context.Context, account *Account, requestedModel string) bool {
	forwardModel, _ := openAIForwardModelFromContext(ctx)
	return s != nil && account != nil && !s.isOpenAIAccountRequestRuntimeBlocked(account, requestedModel, forwardModel.useCompactModelMapping)
}
