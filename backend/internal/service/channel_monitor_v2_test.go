...
func TestChannelMonitorV2ParseFilterDefaultsAndBuckets(t *testing.T) {
	...
	filter, err = svc.ParseFilter("15m", nil, nil, nil)
	require.NoError(t, err)
	require.Equal(t, "15m", filter.Range)
	require.Equal(t, time.Minute, filter.Bucket)
	require.Equal(t, 15*time.Minute, filter.End.Sub(filter.Start))
