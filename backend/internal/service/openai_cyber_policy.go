...
type CyberPolicyMark struct {
	...
	Body           string // 上游 response.failed / 400 body（已截断并脱敏）
...
func MarkOpsCyberPolicy(c *gin.Context, mark CyberPolicyMark) {
	...
	mark.Message = sanitizeUpstreamErrorMessage(strings.TrimSpace(mark.Message))
	mark.Body = strings.TrimSpace(string(sanitizeUpstreamErrorBody([]byte(mark.Body))))
