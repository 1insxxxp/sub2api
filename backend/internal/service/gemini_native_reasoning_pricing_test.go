	...
	for _, provider := range []string{"api_key", "oauth_ai_studio", "oauth_code_assist", "antigravity"} {
		for _, stream := range []bool{false, true} {
			for _, tc := range []struct {
				...
				// Gemini normalizes minimal to low before forwarding; billing follows
				// the effective upstream level.
				{"minimal", map[string]any{"thinkingLevel": "minimal"}, "low", 1.2},
				...
				t.Run(provider+"/"+mode+"/"+tc.name, func(t *testing.T) {
					...
					if provider == "antigravity" && tc.name == "minimal" {
						wantEffort = "minimal"
						wantMultiplier = 1.1
					}
					...
					if strings.EqualFold(requestedLevel, "minimal") && provider != "antigravity" {
						requestedLevel = "low"
					}
