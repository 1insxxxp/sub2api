...
const (
	...
	SettingBalanceRechargePromotion = "BALANCE_RECHARGE_PROMOTION"
...
type PaymentConfig struct {
	...
	BalanceRechargeTiers      []BalanceRechargeTier     `json:"balance_recharge_tiers,omitempty"`
	BalanceRechargePromotion  *BalanceRechargePromotion `json:"balance_recharge_promotion,omitempty"`
...
type UpdatePaymentConfigRequest struct {
	...
	BalanceRechargePromotion  *BalanceRechargePromotion `json:"balance_recharge_promotion"`
	SubscriptionUSDToCNYRate  *float64                  `json:"subscription_usd_to_cny_rate"`
	RechargeFeeRate           *float64                  `json:"recharge_fee_rate"`
	LoadBalanceStrategy       *string                   `json:"load_balance_strategy"`
	ProductNamePrefix         *string                   `json:"product_name_prefix"`
...
func (s *PaymentConfigService) GetPaymentConfig(ctx context.Context) (*PaymentConfig, error) {
	keys := []string{
		...
		SettingEnabledPaymentTypes, SettingBalancePayDisabled, SettingBalanceRechargeMult, SettingBalanceRechargeTiers, SettingBalanceRechargePromotion, SettingSubscriptionUSDToCNYRate, SettingRechargeFeeRate, SettingLoadBalanceStrategy,
...
func (s *PaymentConfigService) parsePaymentConfig(vals map[string]string) *PaymentConfig {
	cfg := &PaymentConfig{
		...
		BalanceRechargePromotion:  parseBalanceRechargePromotion(vals[SettingBalanceRechargePromotion]),
...
func (s *PaymentConfigService) UpdatePaymentConfig(ctx context.Context, req UpdatePaymentConfigRequest) error {
	if req.BalanceRechargeTiers != nil {
		if err := validateBalanceRechargeTiers(*req.BalanceRechargeTiers); err != nil {
			return err
		}
	}
	...
	if req.BalanceRechargeTiers != nil {
		raw, err := json.Marshal(*req.BalanceRechargeTiers)
		if err != nil {
			return err
		}
		m[SettingBalanceRechargeTiers] = string(raw)
	}
	if req.Enabled != nil {
	...
	if req.BalanceRechargePromotion != nil {
		raw, err := json.Marshal(req.BalanceRechargePromotion)
		if err != nil {
			return err
		}
		m[SettingBalanceRechargePromotion] = string(raw)
	}
	if req.SubscriptionUSDToCNYRate != nil {
