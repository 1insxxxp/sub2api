...
func (s *BillingService) getModelPricingWithSource(model string) (*ModelPricing, string, error) {
	return s.getModelPricingWithSourceAt(model, timezone.Now())
}
...
func (s *BillingService) getModelPricingAt(model string, pricingAt time.Time) (*ModelPricing, error) {
	pricing, _, err := s.getModelPricingWithSourceAt(model, pricingAt)
	return pricing, err
}

func (s *BillingService) getModelPricingWithSourceAt(model string, pricingAt time.Time) (*ModelPricing, string, error) {
	// 标准化模型名称（转小写）
	...
	if s.pricingService != nil {
		...
		if litellmPricing != nil {
			...
			}, true, pricingAt), PricingSourceLiteLLM, nil
		}
	}
	...
	if fallback != nil {
		...
		return s.applyModelSpecificPricingPolicyEx(model, fallback, true, pricingAt), PricingSourceFallback, nil
	}
...
// CalculateCostWithLongContext preserves the legacy Gemini v1beta marginal
// long-context rule. Explicit group/channel pricing and catalog ladders use the
// newer unified path instead.
