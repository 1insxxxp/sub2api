...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	modelStatusVisibility, err := normalizeGroupModelStatusVisibility(input.ModelStatusVisibility)
	if err != nil {
		return nil, err
	}
	...
	group := &Group{
		...
		ModelStatusVisibility:            modelStatusVisibility,
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	previousGroup := *group
	previousGroup.ModelAllowlist.Models = append([]string(nil), group.ModelAllowlist.Models...)
	...
	if input.EmptyResponseCompensationEnabled != nil {
		group.EmptyResponseCompensationEnabled = *input.EmptyResponseCompensationEnabled
	}
	...
	if input.ModelStatusVisibility != nil {
		...
		group.ModelStatusVisibility = modelStatusVisibility
	}
	syncGroupModelAllowlistAliases(group)
...
func (s *adminServiceImpl) deleteGroup(ctx context.Context, id int64, requireEmpty bool) error {
	...
	if s.groupSourceRefRepo != nil {
		...
		if referenceCount > 0 {
			return ErrGroupCustomGroupSourceInUse.WithMetadata(map[string]string{
				"reference_count": fmt.Sprintf("%d", referenceCount),
			})
		}
	}
	...
	var deleteErr error
	if requireEmpty {
		affectedUserIDs, deleteErr = s.emptyGroupDeleteRepo.DeleteCascadeIfEmpty(ctx, id)
	} else {
		affectedUserIDs, deleteErr = s.groupRepo.DeleteCascade(ctx, id)
	}
	if deleteErr != nil {
		return deleteErr
	}
