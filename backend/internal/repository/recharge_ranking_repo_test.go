...
func TestGetRechargeRankingAggregatesSourcesAndGuardsDoubleCounting(t *testing.T) {
	...
	item := result.Items[0]
	require.Equal(t, int64(3), item.RechargeCount)
	require.Nil(t, item.InviterID)
...
func mockRowsForRechargeRanking(userID int64, when time.Time) *sqlmock.Rows {
	...
	}).AddRow(userID, "u@example.com", "u", 100.0, 70.0, 10.0, 20.0, 5.0, 6.0, 7.0, 8.0, 14.0, int64(3), when,
		int64(1), 70.0, 10.0, 20.0, 5.0, 6.0, 7.0, 8.0, 14.0, int64(3), int64(1), nil, "", "")
}
