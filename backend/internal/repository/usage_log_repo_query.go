...
func (r *usageLogRepository) GetByID(ctx context.Context, id int64) (log *service.UsageLog, err error) {
	...
	// A transaction-backed repository has only one connection. Release the raw
	// usage row before association hydration issues Ent queries on that same
	// transaction; otherwise rows with a non-nil group_id can interleave wire
	// protocol messages on lib/pq.
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		ThresholdExemptCost:       thresholdExemptCost,
	...
	if customGroupID.Valid {
		value := customGroupID.Int64
		log.CustomGroupID = &value
	}
