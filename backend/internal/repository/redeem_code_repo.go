...
func (r *redeemCodeRepository) DeleteBalanceTransfersByCreator(ctx context.Context, userID int64, codeIDs []int64) ([]service.RedeemCode, error) {
	...
	client := clientFromContext(ctx, r.client)
	rows, err := client.RedeemCode.Query().
		Where(
			redeemcode.IDIn(codeIDs...),
	...
	byID := make(map[int64]service.RedeemCode, len(rows))
	for _, row := range rows {
		byID[row.ID] = *redeemCodeEntityToService(row)
	}

	out := make([]service.RedeemCode, 0, len(codeIDs))
...
func (r *redeemCodeRepository) SumPositiveBalanceByUser(ctx context.Context, userID int64) (float64, error) {
	...
	err := clientFromContext(ctx, r.client).RedeemCode.Query().
		Where(
			...
			redeemcode.ThresholdExemptEQ(false),
		).
...
func (r *redeemCodeRepository) ListByCreator(ctx context.Context, userID int64, limit int) ([]service.RedeemCode, error) {
	if limit <= 0 {
		limit = 25
	}
	codes, err := clientFromContext(ctx, r.client).RedeemCode.Query().
