...
func TestAdminComplianceGuardBypassesSubAdminWorkbenchRoute(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := service.NewSettingService(&complianceGuardRepoStub{}, &config.Config{})
	router := gin.New()
	router.Use(func(c *gin.Context) {
	...
	router.Use(AdminComplianceGuard(svc))
	router.GET("/api/v1/admin/workbench/commission/calendar", func(c *gin.Context) {
		c.String(http.StatusOK, "ok")
	})

	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/workbench/commission/calendar", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
