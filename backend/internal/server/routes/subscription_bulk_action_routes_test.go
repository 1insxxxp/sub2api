...
func TestSubscriptionBulkActionRoutesRequireAdminAuthentication(t *testing.T) {
	...
	workbenchAuth := servermiddleware.AdminWorkbenchAuthMiddleware(func(c *gin.Context) { c.Next() })
	RegisterAdminRoutes(router.Group("/api/v1"), handlers, adminAuth, workbenchAuth, auditLog, stepUp, nil, nil)
