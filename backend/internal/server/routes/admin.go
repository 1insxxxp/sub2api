...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/request-errors/summary", h.Admin.Ops.SummaryRequestErrors)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.POST("/pricing-coverage", h.Admin.Group.PricingCoverage)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/model-alias-renames", h.Admin.Account.CascadeAccountModelAliasRenames)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 认证入口全局 IP 黑名单
		adminSettings.GET("/auth-ip-blacklist", h.Admin.Setting.GetAuthIPBlacklistSettings)
		adminSettings.PUT("/auth-ip-blacklist", h.Admin.Setting.UpdateAuthIPBlacklistSettings)
		// 529过载冷却配置
		...
		// 模型首字超时与换号配置
		adminSettings.GET("/model-first-output-timeout", h.Admin.Setting.GetModelFirstOutputTimeoutSettings)
		adminSettings.PUT("/model-first-output-timeout", h.Admin.Setting.UpdateModelFirstOutputTimeoutSettings)
		// 请求整流器配置
...
func registerChannelRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	channels := admin.Group("/channels")
	{
		...
		channels.GET("/pricing/sync-group-models", h.Admin.Channel.SyncGroupAvailablePricingModels)
...
func registerAffiliateRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		affiliates.GET("/summaries", h.Admin.Affiliate.ListInviterSummaries)
