...
type claimHandlerCompensatorStub struct {
	claimIDs []int64
}

func (s *claimHandlerCompensatorStub) CompensateApprovedClaim(_ context.Context, claimID int64) error {
	s.claimIDs = append(s.claimIDs, claimID)
	return nil
}

func newClaimHandlerTestRouter(repo *claimHandlerRepositoryStub) *gin.Engine {
	return newClaimHandlerTestRouterWithCompensator(repo, nil)
}

func newClaimHandlerTestRouterWithCompensator(repo *claimHandlerRepositoryStub, compensator service.EmptyResponseClaimCompensator) *gin.Engine {
	gin.SetMode(gin.TestMode)
	claimService := service.NewEmptyResponseClaimService(repo, compensator)
