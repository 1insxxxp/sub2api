...
// The UserCheckinStatusSnapshotFunc type is an adapter to allow the use of ordinary
// function as UserCheckinStatusSnapshot mutator.
type UserCheckinStatusSnapshotFunc func(context.Context, *ent.UserCheckinStatusSnapshotMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserCheckinStatusSnapshotFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserCheckinStatusSnapshotMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserCheckinStatusSnapshotMutation", m)
}
