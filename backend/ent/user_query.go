...
type UserQuery struct {
	...
	withCheckinStatusSnapshots  *UserCheckinStatusSnapshotQuery
...
// QueryCheckinStatusSnapshots chains the current query on the "checkin_status_snapshots" edge.
func (_q *UserQuery) QueryCheckinStatusSnapshots() *UserCheckinStatusSnapshotQuery {
	query := (&UserCheckinStatusSnapshotClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCheckinBlacklistEntries chains the current query on the "checkin_blacklist_entries" edge.
func (_q *UserQuery) QueryCheckinBlacklistEntries() *UserCheckinBlacklistQuery {
	query := (&UserCheckinBlacklistClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCheckinStatusSnapshots:  _q.withCheckinStatusSnapshots.Clone(),
...
// WithCheckinStatusSnapshots tells the query-builder to eager-load the nodes that are connected to
// the "checkin_status_snapshots" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCheckinStatusSnapshots(opts ...func(*UserCheckinStatusSnapshotQuery)) *UserQuery {
	query := (&UserCheckinStatusSnapshotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCheckinStatusSnapshots = query
	return _q
}

// WithCheckinBlacklistEntries tells the query-builder to eager-load the nodes that are connected to
// the "checkin_blacklist_entries" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCheckinBlacklistEntries(opts ...func(*UserCheckinBlacklistQuery)) *UserQuery {
	query := (&UserCheckinBlacklistClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCheckinBlacklistEntries = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [17]bool{
			...
			_q.withCheckins != nil,
			_q.withCheckinStatusSnapshots != nil,
	...
	if query := _q.withCheckinStatusSnapshots; query != nil {
		if err := _q.loadCheckinStatusSnapshots(ctx, query, nodes,
			func(n *User) { n.Edges.CheckinStatusSnapshots = []*UserCheckinStatusSnapshot{} },
			func(n *User, e *UserCheckinStatusSnapshot) {
				n.Edges.CheckinStatusSnapshots = append(n.Edges.CheckinStatusSnapshots, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withCheckinBlacklistEntries; query != nil {
		if err := _q.loadCheckinBlacklistEntries(ctx, query, nodes,
...
func (_q *UserQuery) loadCheckinStatusSnapshots(ctx context.Context, query *UserCheckinStatusSnapshotQuery, nodes []*User, init func(*User), assign func(*User, *UserCheckinStatusSnapshot)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usercheckinstatussnapshot.FieldUserID)
	}
	query.Where(predicate.UserCheckinStatusSnapshot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CheckinStatusSnapshotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadCheckinBlacklistEntries(ctx context.Context, query *UserCheckinBlacklistQuery, nodes []*User, init func(*User), assign func(*User, *UserCheckinBlacklist)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usercheckinblacklist.FieldUserID)
	}
