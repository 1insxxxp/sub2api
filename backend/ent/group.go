...
type Group struct {
	...
	// TagColor holds the value of the "tag_color" field.
	TagColor string `json:"tag_color,omitempty"`
	// RateMultiplier holds the value of the "rate_multiplier" field.
	RateMultiplier float64 `json:"rate_multiplier,omitempty"`
	// 是否允许用户对该分组的空回请求申请补偿
	EmptyResponseCompensationEnabled bool `json:"empty_response_compensation_enabled,omitempty"`
	// 是否启用高峰时段倍率
	...
	// 分组模型监控展示范围：仅影响模型状态展示，不限制实际请求
	ModelStatusVisibility domain.GroupModelStatusVisibility `json:"model_status_visibility,omitempty"`
...
// CustomModelRoutesOrErr returns the CustomModelRoutes value or an error if the edge
// was not loaded in eager-loading.
func (e GroupEdges) CustomModelRoutesOrErr() ([]*UserCustomGroupModel, error) {
	if e.loadedTypes[4] {
		return e.CustomModelRoutes, nil
	}
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelAllowlist, group.FieldModelStatusVisibility, group.FieldCodexModelsManifestConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldTagColor:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tag_color", values[i])
			} else if value.Valid {
				_m.TagColor = value.String
			}
		...
		case group.FieldEmptyResponseCompensationEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field empty_response_compensation_enabled", values[i])
			} else if value.Valid {
				_m.EmptyResponseCompensationEnabled = value.Bool
			}
		...
		case group.FieldDefaultReasoningEffort:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case group.FieldModelStatusVisibility:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field model_status_visibility", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ModelStatusVisibility); err != nil {
					return fmt.Errorf("unmarshal field model_status_visibility: %w", err)
				}
			}
...
// QueryCustomModelRoutes queries the "custom_model_routes" edge of the Group entity.
func (_m *Group) QueryCustomModelRoutes() *UserCustomGroupModelQuery {
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("tag_color=")
	builder.WriteString(_m.TagColor)
	builder.WriteString(", ")
	builder.WriteString("rate_multiplier=")
	builder.WriteString(fmt.Sprintf("%v", _m.RateMultiplier))
	builder.WriteString(", ")
	builder.WriteString("empty_response_compensation_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.EmptyResponseCompensationEnabled))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("model_status_visibility=")
	builder.WriteString(fmt.Sprintf("%v", _m.ModelStatusVisibility))
	builder.WriteString(", ")
