...
func (tx *Tx) init() {
	...
	tx.CheckinRewardCampaign = NewCheckinRewardCampaignClient(tx.config)
	tx.CompositeModelRoute = NewCompositeModelRouteClient(tx.config)
	tx.EmptyResponseClaim = NewEmptyResponseClaimClient(tx.config)
	tx.ErrorPassthroughRule = NewErrorPassthroughRuleClient(tx.config)
	tx.Group = NewGroupClient(tx.config)
	tx.IdempotencyRecord = NewIdempotencyRecordClient(tx.config)
	tx.IdentityAdoptionDecision = NewIdentityAdoptionDecisionClient(tx.config)
	tx.LotteryActivity = NewLotteryActivityClient(tx.config)
	tx.LotteryAttemptGrant = NewLotteryAttemptGrantClient(tx.config)
	...
	tx.RedeemBatchClaim = NewRedeemBatchClaimClient(tx.config)
	...
	tx.UsageResponseOutcome = NewUsageResponseOutcomeClient(tx.config)
	...
	tx.UserCustomGroup = NewUserCustomGroupClient(tx.config)
	tx.UserCustomGroupModel = NewUserCustomGroupModelClient(tx.config)
	tx.UserImage = NewUserImageClient(tx.config)
	tx.UserImageTask = NewUserImageTaskClient(tx.config)
