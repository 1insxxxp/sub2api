...
func (_q *APIKeyQuery) QueryCustomGroup() *UserCustomGroupQuery {
...
func (_q *APIKeyQuery) WithCustomGroup(opts ...func(*UserCustomGroupQuery)) *APIKeyQuery {
	query := (&UserCustomGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withCustomGroup; query != nil {
		if err := _q.loadCustomGroup(ctx, query, nodes, nil,
			func(n *APIKey, e *UserCustomGroup) { n.Edges.CustomGroup = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadCustomGroup(ctx context.Context, query *UserCustomGroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *UserCustomGroup)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*APIKey)
	for i := range nodes {
...
func (_q *APIKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCustomGroup != nil {
			_spec.Node.AddColumnOnce(apikey.FieldCustomGroupID)
		}
	}
