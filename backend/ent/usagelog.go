...
type UsageLog struct {
	...
	// Balance-funded cost excluded from eligibility thresholds
	ThresholdExemptCost float64 `json:"threshold_exempt_cost,omitempty"`
	// CompensatedCost holds the value of the "compensated_cost" field.
	CompensatedCost float64 `json:"compensated_cost,omitempty"`
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldInputCost, usagelog.FieldOutputCost, usagelog.FieldCacheCreationCost, usagelog.FieldCacheReadCost, usagelog.FieldTotalCost, usagelog.FieldActualCost, usagelog.FieldThresholdExemptCost, usagelog.FieldCompensatedCost, usagelog.FieldRateMultiplier, usagelog.FieldAccountRateMultiplier:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldThresholdExemptCost:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field threshold_exempt_cost", values[i])
			} else if value.Valid {
				_m.ThresholdExemptCost = value.Float64
			}
		case usagelog.FieldCompensatedCost:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field compensated_cost", values[i])
			} else if value.Valid {
				_m.CompensatedCost = value.Float64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("threshold_exempt_cost=")
	builder.WriteString(fmt.Sprintf("%v", _m.ThresholdExemptCost))
	builder.WriteString(", ")
	builder.WriteString("compensated_cost=")
	builder.WriteString(fmt.Sprintf("%v", _m.CompensatedCost))
	builder.WriteString(", ")
