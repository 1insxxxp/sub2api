<template>
  ...
  >
    <form id="import-data-form" class="space-y-4" @submit.prevent="handleImport">
      <div class="brand-floating-card text-sm leading-6 text-slate-600 dark:text-slate-300">
        {{ t('admin.accounts.dataImportHint') }}
      </div>
      <div
        class="admin-warning-zone px-4 py-3 text-xs leading-5 text-amber-700 dark:text-amber-200"
      >
      ...
      <div>
        ...
        <div
          class="admin-form-section flex items-center justify-between gap-3 border-dashed px-4 py-3 transition-colors sm:px-4 sm:py-3"
          :class="dragActive
        ...
        >
          <div class="min-w-0">
            <div class="truncate text-sm font-medium text-slate-800 dark:text-slate-100" :title="fileListTitle">
      ...
      <div
        v-if="result"
        class="brand-floating-card space-y-2 p-4"
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div
            class="mt-2 max-h-48 overflow-auto rounded-xl border border-rose-100/80 bg-rose-50/60 p-3 font-mono text-xs text-rose-700 dark:border-rose-400/15 dark:bg-rose-500/10 dark:text-rose-200"
          >
