  ...
  it.each(['availableChannels.pricing', 'admin.availableChannels.pricing'])(
    ...
    async (pricingKeyPrefix) => {
      ...
      try {
        ...
        await wrapper.setProps({
          cnyPriceMultiplier: 2,
          model,
        })
        expect(wrapper.findComponent({ name: 'PricingRow' }).text()).toContain(`${pricingKeyPrefix}.currencyCNY0.1 ${pricingKeyPrefix}.unitPerSecond`)
        expect(tooltip?.textContent).toContain(`${pricingKeyPrefix}.currencyCNY0 ${pricingKeyPrefix}.unitPerSecond`)
        expect(tooltip?.textContent).toContain(`${pricingKeyPrefix}.currencyCNY0.24 ${pricingKeyPrefix}.unitPerSecond`)
      } finally {
        wrapper.unmount()
      }
    }
