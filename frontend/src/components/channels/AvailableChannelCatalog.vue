<template>
  <section
    ...
    class="lg:grid lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-6"
  ...
  >
    ...
    <aside
      ...
      class="hidden space-y-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800 lg:block"
    >
      <div v-for="item in 5" :key="item" class="h-20 animate-pulse rounded-xl bg-gray-100 dark:bg-dark-700" />
    </aside>
    <div data-testid="catalog-loading-detail" class="space-y-4">
      <div class="h-32 animate-pulse rounded-2xl bg-gray-100 dark:bg-dark-700" />
      <div class="h-64 animate-pulse rounded-2xl bg-gray-100 dark:bg-dark-700" />
    </div>
  </section>
  ...
  <section
    ...
    class="relative min-w-0 space-y-4 lg:grid lg:grid-cols-[280px_minmax(0,1fr)] lg:items-start lg:gap-6 lg:space-y-0"
    :aria-busy="refreshing"
  >
    <div
      v-if="rateFallback"
      data-testid="rate-fallback-warning"
      class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300 lg:col-span-2"
      role="status"
    >
    ...
    <button
      ...
      class="flex min-h-11 w-full min-w-0 items-center justify-between gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 text-left shadow-sm dark:border-dark-600 dark:bg-dark-800 lg:hidden"
    >
      <span class="min-w-0">
    ...
    <aside
      ...
      class="hidden rounded-2xl border border-gray-200 bg-white p-3 shadow-sm dark:border-dark-600 dark:bg-dark-800 lg:sticky lg:top-4 lg:block lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto"
    >
      <div
        role="listbox"
      ...
      >
        <button
          ...
          class="w-full min-w-0 rounded-xl border px-3 py-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
          ...
          aria-controls="available-channel-detail"
