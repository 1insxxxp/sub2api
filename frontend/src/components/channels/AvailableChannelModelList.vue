...
<template>
  <section v-if="entries.length > 0" data-testid="available-model-list" class="min-w-0">
    ...
    <div data-testid="model-card-grid" class="grid min-w-0 grid-cols-1 gap-4 md:grid-cols-2 2xl:grid-cols-3">
      <article v-for="entry in entries" :key="entry.key" data-testid="model-card" class="model-card-surface group min-w-0 overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm transition-[border-color,box-shadow] duration-200 motion-reduce:transition-none hover:border-primary-200 hover:shadow-md dark:border-dark-600 dark:bg-dark-800 dark:hover:border-primary-500/40">
        <header class="flex min-w-0 items-start gap-3 px-4 pb-3 pt-4 sm:px-5">
        ...
        <template v-if="representative(entry)">
          <div data-testid="model-price-summary" class="mx-4 rounded-xl border border-slate-200/80 bg-slate-50/70 p-3.5 dark:border-dark-600 dark:bg-dark-700/35 sm:mx-5">
            ...
            <div v-if="priceDimensions(representative(entry)!).length" class="mt-3 space-y-1.5 border-t border-slate-200 pt-3 text-xs dark:border-dark-600">
        ...
        <button type="button" data-testid="model-offering-toggle" class="mt-3 flex min-h-11 w-full touch-manipulation items-center justify-between gap-3 border-t border-slate-100 px-4 py-2.5 text-left text-sm font-semibold text-primary-600 transition-colors hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500 motion-reduce:transition-none dark:border-dark-600 dark:text-primary-300 dark:hover:bg-primary-500/10 sm:px-5" :aria-expanded="isExpanded(entry.key)" :aria-controls="detailsId(entry.key)" @click="toggle(entry.key)" @keydown.enter.prevent="toggle(entry.key)" @keydown.space.prevent="toggle(entry.key)">
          <span>{{ isExpanded(entry.key) ? t('availableChannels.catalog.hideOfferings') : t('availableChannels.catalog.showOfferings', { count: entry.offerings.length }) }}</span>
          <svg class="h-4 w-4 shrink-0 transition-transform motion-reduce:transition-none" :class="isExpanded(entry.key) ? 'rotate-180' : ''" viewBox="0 0 20 20" fill="none" aria-hidden="true">
            <path d="m6 8 4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </button>
        <div v-if="isExpanded(entry.key)" :id="detailsId(entry.key)" data-testid="offering-details" class="border-t border-slate-100 bg-slate-50/60 px-3 py-3 dark:border-dark-600 dark:bg-dark-900/20 sm:px-5">
          <div class="space-y-3">
