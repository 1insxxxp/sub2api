<template>
  <div class="flex min-w-0 flex-wrap items-center gap-2">
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" class="group relative mb-1 min-w-0 max-w-full break-inside-avoid">
        <!-- 积分已用尽 -->
        <span
          v-if="item.kind === 'credits_exhausted'"
          class="inline-flex max-w-full min-w-0 flex-wrap items-center gap-1 break-words rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        ...
        <span
          ...
          class="inline-flex max-w-full min-w-0 flex-wrap items-center gap-1 break-words rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
        >
          <span>⚡</span>
        ...
        <span
          v-else
          class="inline-flex max-w-full min-w-0 flex-wrap items-center gap-1 break-words rounded bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-400"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ formatScopeName(item.model) }}
          <span class="text-[10px] opacity-70">{{ formatCountdown(item.reset_at) }}</span>
