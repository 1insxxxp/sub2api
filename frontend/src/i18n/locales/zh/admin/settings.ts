    ...
    settings: {
      ...
      features: {
        ...
        availableChannels: {
          ...
          priceCnyMultiplier: '最低充值比例',
          priceCnyMultiplierHint: '作为可用渠道本站价格区间的下限，仅影响价格展示，不影响真实扣费；填 0 时不展示本站人民币价格区间。',
          priceCnyMultiplierMax: '最高充值比例',
          priceCnyMultiplierMaxHint: '作为可用渠道本站价格区间的上限，默认 0.20；仅影响价格展示，不影响真实扣费。低于最低比例时会自动按最低比例保存。',
          priceCnyPreview: '示例：官方 $10 / 1M token，分组倍率 1x 时展示 ¥{value} / 1M token。',
          officialUsdToCnyRate: '官方价美元兑人民币汇率',
          officialUsdToCnyRateHint: '仅用于计算“省 xx%”：先将官方 USD 定价换算为人民币，再与本站人民币价比较。默认 7；填 0 隐藏优惠标签。',
        },
        siteBillingMode: {
          title: '站点类型',
        ...
        affiliate: {
          ...
          qualificationAmount: '合格充值金额',
          qualificationAmountHint: '被邀请人累计实付达到该金额后计为合格邀请。',
          tiers: {
            title: '推广等级',
            threshold: '合格邀请人数',
            rate: '返利比例',
            standardHint: '基础等级',
            hint: '等级按合格邀请人数自动计算；用户专属比例仍会覆盖自动等级比例。',
      ...
      customMenu: {
        title: '自定义菜单页面',
        description: '添加自定义页面到侧边栏导航，并选择站内嵌入或新标签页打开。每个页面可以设置为普通用户或管理员可见。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
        ...
        visibilityAdmin: '管理员',
        openMode: '打开方式',
        openModeEmbedded: '站内嵌入',
        openModeNewTab: '新标签页',
        openModeHint: '新标签页模式直接打开原始 URL，不会附加用户 ID、登录令牌或嵌入参数。',
        openModeMarkdownHint: 'Markdown 页面只能在站内打开。',
        add: '添加菜单项',
        remove: '删除',
        moveUp: '上移',
        moveDown: '下移',
      ...
      payment: {
        ...
        rechargePromotion: {
          title: '充值优惠活动',
          hint: '单个定时活动会替换普通充值倍率；黑名单用户仍按普通价格充值。',
          name: '活动名称',
          namePlaceholder: '例如：周年庆',
          startAt: '开始时间',
          endAt: '结束时间',
          multiplier: '活动倍率',
          multiplierHint: '启用活动时必须大于 0。',
          blacklist: '普通价格用户',
          blacklistHint: '可按邮箱或用户 ID 搜索；名单中的用户仍使用普通倍率。',
          searchUsers: '按邮箱或用户 ID 搜索',
          noUsers: '没有找到用户',
          userFallback: '用户 #{id}',
          removeUser: '移除用户',
          invalidMultiplier: '请输入大于 0 的活动倍率。',
          invalidRange: '结束时间必须晚于开始时间。',
        },
        balanceRechargeMultiplier: '余额充值倍率',
        rechargeTiers: '快捷充值金额',
        addRechargeTier: '添加快捷金额',
        rechargeTiersHint: '用户只能选择预设充值金额，每个金额使用独立倍率；未配置时使用全局倍率。',
        tierAmount: '快捷金额',
        tierMultiplierPlaceholder: '填写倍率',
        invalidRechargeTiers: '请填写大于 0 且不重复的快捷金额，并为每个金额填写正数倍率。',
      ...
      modelFirstOutputTimeout: {
        ...
        defaultTitle: '全局默认策略',
        globalPolicy: '全局策略',
        profileTitle: 'Gemini 配置档',
        profileHint: '为 Flash、Pro 和 Thinking 配置独立策略。',
        platformTitle: '平台覆盖',
        platformHint: '可为平台设置覆盖策略，例如 gemini 或 openai。',
        modelTitle: '模型覆盖',
        modelHint: '可为具体模型设置覆盖策略，优先级高于平台和默认策略。',
        addOverride: '添加覆盖',
        removeOverride: '移除覆盖',
        noOverrides: '暂无覆盖项',
        platformPlaceholder: '平台名称',
        modelPlaceholder: '模型名称',
        targetSeconds: '目标时间（秒）',
        switchSeconds: '切换时间（秒）',
        hardCapSeconds: '硬上限（秒）',
        retryHint: '仅适用于首个输出；超时后最多重试一次。有效范围为 1-900 秒，且目标时间 ≤ 切换时间 ≤ 硬上限。',
        validationPositive: '所有超时值必须为 1-900 秒以内的正数。',
        validationOrder: '超时值必须满足目标时间 ≤ 切换时间 ≤ 硬上限。',
        loadFailed: '加载首个输出超时设置失败',
        saved: '首个输出超时设置保存成功',
        saveFailed: '保存首个输出超时设置失败',
        profile: {
