        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{{ creditedAmountSymbol }}{{ row.refund_amount.toFixed(2) }}</span>
