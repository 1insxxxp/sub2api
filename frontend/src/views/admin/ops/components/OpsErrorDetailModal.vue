  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="admin-form-section !space-y-1 !p-4">
        ...
        <div v-if="detail.api_key_prefix" class="admin-form-section !space-y-1 !p-4">
        ...
        <div v-if="detail.attempted_key_prefix" class="admin-form-section !space-y-1 !p-4">
        ...
        <div v-if="detail.deleted_key_owner_email" class="admin-form-section !space-y-1 !p-4">
      ...
      <div v-if="rootCauseMessage" class="admin-form-section !space-y-3 !border-amber-200/70 !bg-amber-50/80 !p-6 dark:!border-amber-500/20 dark:!bg-amber-900/10">
        ...
        <div class="break-words text-sm font-medium text-amber-900 dark:text-amber-100">{{ rootCauseMessage }}</div>
      </div>

      <div class="admin-form-section !space-y-4 !p-6">
        ...
        <div v-if="!diagnosticPayloadSections.length" class="text-sm text-gray-500 dark:text-gray-400">{{ t('common.noData') }}</div>
        <div v-else class="space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            ...
            <pre class="max-h-[520px] overflow-auto rounded-xl border border-primary-100/70 bg-white/75 p-4 text-xs text-gray-800 dark:border-primary-400/10 dark:bg-white/[0.04] dark:text-gray-100"><code>{{ prettyJSON(section.value) }}</code></pre>
          </div>
        </div>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="admin-list-surface p-6">
        ...
        <div v-else class="mt-4 space-y-3">
          <div
            ...
            :key="ev.id"
            class="admin-list-row rounded-xl border border-primary-100/60 bg-white/70 p-4 dark:border-primary-400/10 dark:bg-white/[0.03]"
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="admin-inline-action !min-h-7 !min-w-0 !flex-row !gap-1.5 !rounded-md !px-1.5 !py-1 !text-[10px] !font-bold disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <pre
              ...
              class="admin-form-section mt-3 max-h-[240px] overflow-auto !space-y-0 !rounded-xl !p-3 text-xs text-gray-800 dark:text-gray-100"
