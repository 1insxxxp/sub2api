...
describe('OpsUpstreamErrorSummaryModal', () => {
  it('renders a dense table with model, account, reason and status rows', async () => {
    ...
    expect(wrapper.find('[data-testid=ops-summary-row-77]').exists()).toBe(false)
    await accountToggle.trigger('click')
    expect(wrapper.text()).toContain('model-long'); expect(wrapper.text()).toContain('upstream down'); expect(wrapper.text()).toContain('502'); expect(wrapper.text()).toContain('×2')
  ...
  it('paginates each group with ten latest rows by default', async () => {
    ...
    const wrapper = mountModal(); await nextTick(); await nextTick()
    await wrapper.find('button[aria-expanded="false"]').trigger('click'); await nextTick()
    ...
    expect(wrapper.find('[data-testid="ops-summary-pagination-0"]').classes()).toContain('ops-summary-pagination')
