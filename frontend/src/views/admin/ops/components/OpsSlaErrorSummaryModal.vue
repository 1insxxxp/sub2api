      ...
      <template v-else>
        ...
        <div class="space-y-3">
          <section v-for="(group, gi) in summary.groups" :key="`${group.group_id ?? 'none'}-${gi}`" class="admin-list-surface overflow-hidden rounded-lg" :data-testid="`ops-sla-summary-group-${gi}`">
            ...
            <div class="space-y-2 px-2 py-2" :data-testid="`ops-sla-summary-table-${gi}`">
              <section v-for="accountGroup in accountGroupsForGroup(group, gi)" :key="`${accountGroup.accountId ?? 'unknown'}-${accountGroup.account}`" class="overflow-hidden rounded-md border border-gray-200/70 dark:border-dark-700">
                <button type="button" class="flex w-full items-center gap-2 bg-gray-50/80 px-3 py-2 text-left text-xs hover:bg-primary-50/50 dark:bg-dark-900/40 dark:hover:bg-primary-500/10" :aria-expanded="!isAccountCollapsed(group, gi, accountGroup)" @click="toggleAccount(group, gi, accountGroup)">
                  <span class="rounded bg-primary-50 px-1.5 py-0.5 text-[10px] font-semibold text-primary-700 dark:bg-primary-500/10 dark:text-primary-300">{{ t('admin.ops.errorDetails.slaSummaryAccount') }}</span>
                  <span class="min-w-0 flex-1 truncate font-semibold text-gray-800 dark:text-gray-100">{{ accountGroup.account }}</span>
                  <span class="whitespace-nowrap text-[10px] text-gray-500 dark:text-gray-400">×{{ accountGroup.totalCount }} {{ t('admin.ops.errorDetails.slaSummaryErrorsShort') }}</span>
