...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.errorDetails.slaSummaryTitle')" width="extra-wide" appearance="neutral" @close="emit('close')">
    <div class="space-y-3">
      <div v-if="loading" class="flex flex-col items-center justify-center py-16 text-sm text-gray-500 dark:text-gray-400"><div class="mb-3 h-8 w-8 animate-spin rounded-full border-b-2 border-primary-600" />{{ t('admin.ops.errorDetails.slaSummaryLoading') }}</div>
      <div v-else-if="error" class="rounded-xl border border-red-200/70 bg-red-50/70 p-6 text-center text-sm text-red-700 dark:border-red-500/20 dark:bg-red-900/20 dark:text-red-300"><p>{{ t('admin.ops.errorDetails.slaSummaryError') }}</p><button type="button" class="admin-inline-action mt-3 !min-h-9 !px-3" @click="load">{{ t('admin.ops.errorDetails.slaSummaryRetry') }}</button></div>
      <div v-else-if="!summary || !summary.total_errors" class="py-16 text-center text-sm text-gray-500 dark:text-gray-400">{{ t('admin.ops.errorDetails.slaSummaryEmpty') }}</div>
      <template v-else>
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <div class="admin-form-section !space-y-1 !p-3"><div class="text-[10px] font-bold uppercase text-gray-400">{{ t('admin.ops.errorDetails.slaSummaryTotalErrors') }}</div><div class="text-lg font-black text-gray-900 dark:text-white">{{ summary.total_errors }}</div></div>
          <div class="admin-form-section !space-y-1 !p-3"><div class="text-[10px] font-bold uppercase text-gray-400">{{ t('admin.ops.errorDetails.slaSummaryGroups') }}</div><div class="text-lg font-black text-gray-900 dark:text-white">{{ summary.group_count }}</div></div>
          <div class="admin-form-section !space-y-1 !p-3"><div class="text-[10px] font-bold uppercase text-gray-400">{{ t('admin.ops.errorDetails.slaSummaryLatest') }}</div><div class="break-words text-sm font-semibold text-gray-900 dark:text-white">{{ formatDate(summary.latest_at) }}</div></div>
          <div v-if="summary.groups_truncated" class="col-span-2 rounded-xl border border-amber-200/70 bg-amber-50/70 p-3 text-xs text-amber-800 dark:border-amber-500/20 dark:bg-amber-900/20 sm:col-span-1">{{ t('admin.ops.errorDetails.slaSummaryTruncated') }}</div>
        </div>
        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border border-primary-200/70 bg-primary-50/60 px-3 py-2 text-xs text-primary-800 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200"><span class="font-semibold">{{ t('admin.ops.errorDetails.slaSummaryPath') }}</span><span class="text-primary-700/80 dark:text-primary-200/80">{{ t('admin.ops.errorDetails.slaSummaryPathHint') }}</span></div>
        <div class="space-y-3">
          <section v-for="(group, gi) in summary.groups" :key="`${group.group_id ?? 'none'}-${gi}`" class="admin-list-surface overflow-hidden rounded-lg" :data-testid="`ops-sla-summary-group-${gi}`">
            ...
            <div class="overflow-x-auto"><table class="min-w-[760px] w-full text-xs" :data-testid="`ops-sla-summary-table-${gi}`"><thead class="bg-gray-50/80 text-left text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:bg-dark-900/40 dark:text-gray-400"><tr><th class="w-[24%] px-3 py-2">{{ t('admin.ops.errorDetails.slaSummaryModel') }} / {{ t('admin.ops.errorDetails.slaSummaryAccount') }}</th><th class="w-[34%] px-3 py-2">{{ t('admin.ops.errorDetails.slaSummaryReason') }}</th><th class="px-3 py-2">{{ t('admin.ops.errorDetails.slaSummaryStatus') }}</th><th class="px-3 py-2">{{ t('admin.ops.errorDetails.slaSummaryCount') }}</th><th class="px-3 py-2">{{ t('admin.ops.errorDetails.slaSummaryLatestTime') }}</th><th class="px-3 py-2 text-right">{{ t('admin.ops.errorDetails.slaSummaryAction') }}</th></tr></thead><tbody class="divide-y divide-gray-200/70 dark:divide-dark-700"><tr v-for="row in pagedRowsForGroup(group, gi)" :key="`${row.id}-${row.model}-${row.account}`" class="align-top hover:bg-primary-50/30 dark:hover:bg-primary-500/5" :data-testid="`ops-sla-summary-row-${row.id}`"><td class="max-w-[220px] break-words px-3 py-2.5"><div class="font-semibold text-gray-800 dark:text-gray-100">{{ row.model }}</div><div class="mt-1 border-l-2 border-primary-200 pl-2 text-[11px] text-gray-600 dark:border-primary-500/30 dark:text-gray-300"><span class="mr-1 rounded bg-primary-50 px-1 py-0.5 text-[10px] text-primary-700 dark:bg-primary-500/10 dark:text-primary-300">↳ {{ t('admin.ops.errorDetails.slaSummaryAccount') }}</span>{{ row.account }}</div></td><td class="max-w-[360px] px-3 py-2.5"><span class="block max-w-[360px] truncate font-medium text-gray-700 dark:text-gray-200" :title="row.message">{{ row.message || t('common.noData') }}</span><span class="mt-1 inline-flex rounded-md bg-gray-100 px-1.5 py-0.5 font-mono text-[10px] text-gray-500 dark:bg-dark-700 dark:text-gray-300">{{ row.errorType }}</span></td><td class="whitespace-nowrap px-3 py-2.5"><span class="rounded-md px-1.5 py-0.5 font-mono text-[10px]" :class="statusClass(row.statusCode)">{{ row.statusCode }}</span></td><td class="whitespace-nowrap px-3 py-2.5 font-semibold text-gray-700 dark:text-gray-200">×{{ row.count }}</td><td class="whitespace-nowrap px-3 py-2.5 text-gray-500 dark:text-gray-400">{{ formatDate(row.latestAt) }}</td><td class="whitespace-nowrap px-3 py-2.5 text-right"><button type="button" class="font-medium text-primary-600 hover:underline dark:text-primary-400" @click="emit('openErrorDetail', row.id)">{{ t('admin.ops.errorDetails.slaSummaryAction') }}</button></td></tr><tr v-if="!rowsForGroup(group).length"><td colspan="6" class="px-3 py-6 text-center text-gray-500">{{ t('common.noData') }}</td></tr></tbody></table></div><div v-if="totalPagesForGroup(group) > 1" :data-testid="`ops-sla-summary-pagination-${gi}`" class="border-t border-gray-200/70 dark:border-dark-700"><Pagination :total="rowsForGroup(group).length" :page="pageForGroup(group, gi)" :page-size="GROUP_PAGE_SIZE" :show-page-size-selector="false" variant="compact" @update:page="setGroupPage(group, gi, $event)" /></div>
