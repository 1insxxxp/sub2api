...
async function load() {
  ...
  try {
    ...
    summary.value = data
    expandedGroups.value = new Set((data.groups || []).map((g, i) => groupKey(g, i)))
  ...
  <BaseDialog :show="show" :title="t('admin.ops.errorDetails.summaryTitle')" width="extra-wide" appearance="neutral" @close="close">
    <div class="space-y-4">
      ...
      <template v-else>
        ...
        <div class="space-y-2">
          <div v-for="(group, gi) in summary.groups" :key="groupKey(group, gi)" class="admin-list-surface overflow-hidden rounded-xl">
            <button type="button" class="flex w-full items-center justify-between gap-3 px-4 py-3 text-left" @click="toggleGroup(groupKey(group, gi))"><span class="min-w-0 break-words font-bold text-gray-900 dark:text-white">{{ group.group_name }} <span class="ml-1 text-xs font-medium text-gray-500">{{ group.error_count }}</span></span><span class="text-xs text-gray-500"><span :aria-label="isGroupExpanded(groupKey(group, gi)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isGroupExpanded(groupKey(group, gi)) ? '−' : '+' }}</span></span></button>
            <div v-if="isGroupExpanded(groupKey(group, gi))" class="space-y-2 border-t border-gray-200/70 p-3 dark:border-dark-700">
              <div v-for="model in group.models" :key="model.model" class="rounded-lg border border-gray-200/70 bg-white/50 dark:border-dark-700 dark:bg-white/[0.02]">
                <button type="button" class="flex w-full items-center justify-between gap-3 px-3 py-2 text-left" @click="toggleModel(modelKey(group, model, gi))"><span class="min-w-0 break-words font-semibold text-gray-800 dark:text-gray-100">{{ model.model }} <span class="ml-1 text-xs font-medium text-gray-500">{{ model.error_count }}</span></span><span class="text-xs text-gray-500"><span :aria-label="isModelExpanded(modelKey(group, model, gi)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isModelExpanded(modelKey(group, model, gi)) ? '−' : '+' }}</span></span></button>
                <div v-if="isModelExpanded(modelKey(group, model, gi))" class="space-y-2 border-t border-gray-200/60 p-2 dark:border-dark-700">
                  <div v-for="(account, ai) in model.accounts" :key="accountKey(group, model, account, gi, ai)" class="rounded-lg border border-gray-200/60 p-2 dark:border-dark-700">
                    <button type="button" class="flex w-full items-center justify-between gap-3 text-left" @click="toggleAccount(accountKey(group, model, account, gi, ai))"><span class="min-w-0 break-words text-sm font-semibold text-gray-800 dark:text-gray-100">{{ account.account_name }} <span class="ml-1 text-xs text-gray-500">{{ account.error_count }}</span></span><span class="text-xs text-gray-500"><span :aria-label="isAccountExpanded(accountKey(group, model, account, gi, ai)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isAccountExpanded(accountKey(group, model, account, gi, ai)) ? '−' : '+' }}</span></span></button>
                    <div v-if="isAccountExpanded(accountKey(group, model, account, gi, ai))" class="mt-2 space-y-2">
                      <div v-for="reason in account.reasons" :key="`${reason.representative_error_id}-${reason.message}`" class="rounded-lg bg-gray-50/80 p-3 text-xs dark:bg-dark-900/50"><div class="flex flex-wrap items-center gap-2"><span class="rounded-md bg-gray-200/70 px-1.5 py-0.5 font-mono dark:bg-dark-700">{{ reason.error_type }}</span><span class="rounded-md bg-gray-200/70 px-1.5 py-0.5 font-mono dark:bg-dark-700">{{ reason.status_code }}</span><span class="text-gray-500">×{{ reason.count }}</span><span class="ml-auto text-gray-400">{{ formatDate(reason.latest_at) }}</span></div><div class="mt-2 break-words whitespace-pre-wrap text-gray-700 dark:text-gray-200">{{ reason.message || t('common.noData') }}</div><button type="button" class="mt-2 font-mono text-primary-600 underline underline-offset-2 dark:text-primary-400" @click="openDetail(reason.representative_error_id)">{{ t('admin.ops.errorDetails.summaryOpenError', { id: reason.representative_error_id }) }}</button></div>
                    </div>
                  </div>
