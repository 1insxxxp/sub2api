      ...
      <template v-else>
        ...
        <div class="space-y-2">
          <div v-for="(group, gi) in summary.groups" :key="groupKey(group, gi)" class="admin-list-surface overflow-hidden rounded-xl">
            <button type="button" class="flex w-full items-center justify-between gap-3 px-4 py-3 text-left" @click="toggleGroup(groupKey(group, gi))"><span class="min-w-0 break-words font-bold text-gray-900 dark:text-white">{{ group.group_name }} <span class="ml-1 text-xs font-medium text-gray-500">{{ group.error_count }}</span> <span class="ml-2 text-[10px] font-normal text-gray-400">{{ formatDate(group.latest_at) }}</span></span><span class="text-xs text-gray-500"><span :aria-label="isGroupExpanded(groupKey(group, gi)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isGroupExpanded(groupKey(group, gi)) ? '−' : '+' }}</span></span></button>
