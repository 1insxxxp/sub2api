  ...
  <BaseDialog :show="show" :title="t('admin.ops.errorDetails.summaryTitle')" width="extra-wide" appearance="neutral" @close="close">
    <div class="space-y-3">
      ...
      <template v-else>
        ...
        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border border-primary-200/70 bg-primary-50/60 px-3 py-2 text-xs text-primary-800 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200">
          <span class="font-semibold">{{ t('admin.ops.errorDetails.summaryPath') }}</span>
          <span class="text-primary-700/80 dark:text-primary-200/80">{{ t('admin.ops.errorDetails.summaryPathHint') }}</span>
        </div>
        <div class="space-y-1.5">
          <div v-for="(group, gi) in summary.groups" :key="groupKey(group, gi)" class="admin-list-surface overflow-hidden rounded-lg">
            <button type="button" class="flex w-full flex-wrap items-center justify-between gap-x-3 gap-y-1 px-3 py-2.5 text-left" :aria-expanded="isGroupExpanded(groupKey(group, gi))" :aria-controls="nodeId('group', groupKey(group, gi))" @click="toggleGroup(groupKey(group, gi))">
              <span class="min-w-0 flex-1 break-words text-sm font-bold text-gray-900 dark:text-white">{{ group.group_name }}</span>
              <span class="flex flex-wrap items-center gap-1.5 text-[10px] font-medium text-gray-500 dark:text-gray-400">
                <span class="rounded-md bg-rose-50 px-1.5 py-0.5 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{{ group.error_count }} {{ t('admin.ops.errorDetails.summaryErrorsShort') }}</span>
                <span class="rounded-md bg-gray-100 px-1.5 py-0.5 dark:bg-dark-700">{{ group.model_count }} {{ t('admin.ops.errorDetails.summaryModelsShort') }}</span>
                <span class="rounded-md bg-gray-100 px-1.5 py-0.5 dark:bg-dark-700">{{ group.account_count }} {{ t('admin.ops.errorDetails.summaryAccountsShort') }}</span>
                <span class="text-gray-400">{{ formatDate(group.latest_at) }}</span>
              </span>
              <span class="text-xs text-gray-500"><span :aria-label="isGroupExpanded(groupKey(group, gi)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isGroupExpanded(groupKey(group, gi)) ? '−' : '+' }}</span></span>
            </button>
