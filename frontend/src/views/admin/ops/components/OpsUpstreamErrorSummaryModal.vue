      ...
      <template v-else>
        ...
        <div class="space-y-2">
          <div v-for="(group, gi) in summary.groups" :key="groupKey(group, gi)" class="admin-list-surface overflow-hidden rounded-xl">
            <button type="button" class="flex w-full items-center justify-between gap-3 px-4 py-3 text-left" :aria-expanded="isGroupExpanded(groupKey(group, gi))" :aria-controls="nodeId('group', groupKey(group, gi))" @click="toggleGroup(groupKey(group, gi))"><span class="min-w-0 break-words font-bold text-gray-900 dark:text-white">{{ group.group_name }} <span class="ml-1 text-xs font-medium text-gray-500">{{ group.error_count }}</span> <span class="ml-2 text-[10px] font-normal text-gray-400">{{ formatDate(group.latest_at) }}</span></span><span class="text-xs text-gray-500"><span :aria-label="isGroupExpanded(groupKey(group, gi)) ? t('admin.ops.errorDetails.summaryCollapse') : t('admin.ops.errorDetails.summaryExpand')">{{ isGroupExpanded(groupKey(group, gi)) ? '−' : '+' }}</span></span></button>
            <div v-if="isGroupExpanded(groupKey(group, gi))" :id="nodeId('group', groupKey(group, gi))" class="space-y-2 border-t border-gray-200/70 p-3 dark:border-dark-700">
              <div v-for="model in group.models" :key="model.model" class="rounded-lg border border-gray-200/70 bg-white/50 dark:border-dark-700 dark:bg-white/[0.02]">
                ...
                <div v-if="isModelExpanded(modelKey(group, model, gi))" :id="nodeId('model', modelKey(group, model, gi))" class="space-y-2 border-t border-gray-200/60 p-2 dark:border-dark-700">
                  <div v-for="(account, ai) in model.accounts" :key="accountKey(group, model, account, gi, ai)" class="rounded-lg border border-gray-200/60 p-2 dark:border-dark-700">
                    ...
                    <div v-if="isAccountExpanded(accountKey(group, model, account, gi, ai))" :id="nodeId('account', accountKey(group, model, account, gi, ai))" class="mt-2 space-y-2">
