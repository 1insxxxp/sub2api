<template>
    <div class="backup-admin-page space-y-6">
      ...
      <!-- Schedule Config -->
      <section data-test="backup-schedule-surface" data-testid="backup-schedule" class="admin-surface overflow-hidden">
        <div class="admin-panel-header">
          <div class="flex min-w-0 items-start gap-3">
            <div class="backup-icon-shell bg-blue-100 text-blue-600 dark:bg-blue-500/15 dark:text-blue-300">
        ...
        <div class="grid grid-cols-1 gap-4 p-5 md:grid-cols-2">
          ...
          <div class="space-y-4 md:col-span-2">
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.backup.schedule.ordinaryHint') }}</p>
            <BackupArchiveSettings v-model="archiveForm" />
            <p v-if="scheduleValidationError" class="text-sm text-red-600 dark:text-red-400" role="alert">{{ scheduleValidationError }}</p>
            <div class="text-sm text-gray-700 dark:text-gray-300" aria-live="polite">
    ...
    <teleport to="body">
      <transition name="modal">
        <div
          ...
          class="brand-overlay z-50 flex items-center justify-center p-4"
          @mousedown.self="closeDownloadParts"
        >
          <div class="brand-floating-panel relative max-h-[85vh] w-full max-w-lg overflow-y-auto p-6">
            <button
              type="button"
