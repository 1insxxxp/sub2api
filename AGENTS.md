...
### 用户页面统一基准（2026-09-19 用户确认）

- 已确认的 API 密钥页是后续用户页面的视觉基准。沿用 `KeysView.vue`、`KeyMobileCard.vue`、`AppHeader.vue` 的中性半透明表面、极淡斜向渐变、内侧高光、细边框、柔和阴影和紧凑布局。
- 使用既有字体与图标系统；品牌蓝集中用于主要操作和选中态，厂商色用于小图标和微弱点缀。卡片小圆角，工具栏和页面分区保持无外框，避免层层套卡片。
- 浅深色同步适配，移动端按 H5 网页设计，动效短促克制并支持减少动态效果。保留页面功能、接口与字段，按页面逐步接入，不全站覆盖上游通用组件。
- 首批接入仪表盘与使用记录。共用样式位于 `frontend/src/styles/user-workspace.css`，仅在带 `user-workspace` 的页面内生效；后续页面按相同规则逐页验证。

### 管理员页面统一基准（2026-09-19 用户确认）

- 管理员端沿用已确认的 API 密钥页风格，保留后台表格的信息密度、状态颜色和业务操作。共用材质变量位于 `frontend/src/styles/workspace-tokens.css`。
- 管理路由通过 `App.vue` 单独启用 `admin-console-theme`，样式集中在 `frontend/src/styles/admin-workspace.css`，包括传送到 body 的菜单；离开管理路由后移除此标识，避免影响用户页。
- 通过外观上下文让后台 `BaseDialog` 默认使用中性样式、`Pagination` 默认使用紧凑样式，显式传入的外观参数优先。不要为视觉统一改动接口、权限、计费或提交逻辑。
- 首批实际布局重构覆盖管理员仪表盘、账号、用户、分组以及独立的“管理员页面”工作台五页；其他后台页面目前仅统一了基础样式，不视为已完成逐页重构。
- “管理员页面”工作台沿用紧凑页签，页面内不重复显示余额栏，保留转赠余额校验；返利设置、日历与历史记录采用无外框分区，转赠表单和生成结果作为功能工具保留统一材质；移动端历史记录的复制、删除使用 44px 图标按钮，不扩展为整行按钮。
- 列表页使用 `AdminListToolbar.vue` 分离搜索、主要操作和筛选，H5 默认收起筛选并显示已选条件数量。`DataTable` 的 `mobileLayout` 为显式接入：复用原单元格插槽展示身份、状态和核心数据，其余可见字段放入原生详情展开区，复选框和操作保持常显；未接入页面不改变原布局。

### 卡片质感（2026-09-19 用户确认）

- 用户偏好分组选择卡片的表面质感，后续卡片改造以 `frontend/src/components/common/GroupOptionItem.vue` 为参考：中性底色、极淡斜向渐变、内侧高光、细边框与柔和阴影，浅深模式同步适配。
- 密钥卡片沿用这一质感，保留现有紧凑布局、信息层级和操作；微弱点缀色跟随厂商，独立于 tag 颜色，避免大面积染色或装饰色条。

### 轻量操作菜单（2026-09-19 用户确认）

- 后续其他页面的“更多操作”、少量快捷操作入口，优先沿用 API 密钥页的轻量菜单方式，参考 `frontend/src/views/user/KeysView.vue` 的 `keys-mobile-tools`。
- 菜单紧贴触发按钮展开，宽度适应内容和视口；采用中性半透明底色、细边框、柔和阴影、小圆角，统一适配浅色与深色模式。
- 动效保持短促克制，使用轻微淡入和位移，并尊重系统减少动态效果的设置。
- 轻量菜单不添加大标题区、独立关闭按钮、整屏遮罩或大面积彩色渐变；支持再次点击触发按钮、点击外部、Esc、Tab 收起，并正确处理键盘焦点与滚动定位。
- 表单、复杂设置、确认操作按实际需要保留对话框；在逐页重构时采用本约定，不进行全站批量替换。移动端按网页 H5 设计，保持现有业务功能与接口，尽量减少与上游代码的冲突。
...
### 图生图参考图预览配置修复（2026-09-19）

- 线上 `config.yaml` 的自定义 CSP 缺少 `img-src blob:`，导致 Image Studio 使用 `URL.createObjectURL` 的参考图缩略图被浏览器拦截。已在 `/opt/sub2api-custom-prod/deploy/data/config.yaml` 的 `img-src` 中补上 `blob:`。
- 本次为配置修复，应用镜像仍为 `sub2api-custom:0.2.6-0597f9c1b`。当前入口实例为 `sub2api-api-026-image-preview-20260919`（`127.0.0.1:18192`）和 `sub2api-direct-026-image-preview-20260919`（`127.0.0.1:18193`）；环境变量、挂载与网络沿用对应原实例。
- Nginx 两个入口已平滑切换，原 `sub2api-api-026-0597f9c1b`（18190）和 `sub2api-direct-026-0597f9c1b`（18191）保留供回退。部署前仍须读取实际路由，不硬编码上述端口。
- 配置、Nginx 路由和原容器参数备份位于 `/opt/backups/sub2api-image-preview-20260919`。目录含敏感配置，只供服务器运维使用；回退前核对是否有后续配置变更。
- 仓库安全头中间件已补充旧 CSP 的兼容逻辑和回归测试，尚未随新镜像发布；后续发布按正常 CI 门禁执行。
...
分组支持自定义展示标签 `tag` 和颜色 `tag_color`，普通分组及系统自定义分组均适用：
- `tag`：UTF-8 文字，去掉首尾空白后最多 20 个 Unicode 码点；允许空值，不允许控制字符或换行。
- `tag_color`：`#RRGGBB` 十六进制颜色（大小写均可），或 `""` 使用默认配色。
- 旧标签 `chat`（对话）、`image`（生图）、`airp`（AIRP）保持兼容；未设置颜色时沿用默认配色。
- 创建时两字段均可省略；更新时省略字段保留原值，显式 `""` 清空该字段；清空标签同时清空颜色。
例如 `PUT /api/v1/admin/groups/123` 的请求体 `{"tag":"高速线路","tag_color":"#14B8A6"}` 修改标签及颜色。
标签会在管理列表、分组选择及用户侧分组卡片展示，不影响调度和计费。

```bash
curl -X PUT "${BASE}/api/v1/admin/groups/123" \
  -H "x-api-key: ${KEY}" -H "Content-Type: application/json" \
  -d '{"tag":"高速线路","tag_color":"#14B8A6"}'
```
